...
export const Manage = () => {
  ...
  const { cowCode, cowName, cowBreed, cowDate, cowWeight, cowChilds } =
    formState;
  ...
  const onSubmitForm = async (e) => {
    if (disableForm) {
      ...
      if (verifyInputs) {
        const res = await postCowsRequest(
          formState,
  ...
  const onEditData = (e) => {
    if (disableForm) {
      ...
      async function putData() {
        ...
        const res = await putCowsRequest(
          data,
          document.cookie.replace("token=", "")
        );
        const toast = Swal.mixin({
  ...
  const onDeleteData = (cowID) => {
    if (disableForm) {
      ...
      async function deleteData() {
        const res = await deleteCowsRequest(
          cowID,
          document.cookie.replace("token=", "")
        );
        const toast = Swal.mixin({
  ...
  return (
    <>
      <p className="text-4xl text-gray-800 font-semibold mt-4 ml-8">
        Gestionar
      </p>
